...
int main()
{
    ...
    serverInfo.sin_port = htons(SVPORT);
    ...
    {   
        ...
        clientInfo.sin_port = htons(CLPORT);
        clientInfo.sin_addr.s_addr = inet_addr(machineIP);

        int bindRes = bind(sockfd, (struct sockaddr *) &clientInfo, sizeof(clientInfo));
        if(bindRes != 0)
        {
        ...
        socklen_t svLen = sizeof(serverInfo);
        connfd = accept(sockfd, (struct sockaddr *) &serverInfo, &svLen);
        sockfd = connfd;
    }
    else
    {
